"""
Run this once to ingest all Java tutorial docs into the java_tutor ChromaDB collection.

Usage:
    python ingest_java_docs.py
"""
...
JAVA_DOCS_DIR = os.path.join(BASE_DIR, "docs", "java")
...
print(f"Looking for Java docs in: {JAVA_DOCS_DIR}")

md_files = [f for f in os.listdir(JAVA_DOCS_DIR) if f.endswith(".md")]
print(f"Found {len(md_files)} markdown files.")

total_chunks = 0

for filename in sorted(md_files):
    filepath = os.path.join(JAVA_DOCS_DIR, filename)
    ...
    ids = [f"java_{source_id}_{i}" for i in range(len(chunks))]
    ...
    collection.add(
        ...
        metadatas=[{"source": filename, "language": "java"}] * len(new_indices),
